#!/bin/bash
# Update U-Boot submodule to latest stable tag
# Usage: ./scripts/update-uboot.sh [--tag <tag>]
...
# Configuration
UBOOT_SOURCE_DIR="${PROJECT_ROOT}/sources/u-boot"
LOG_DIR="${PROJECT_ROOT}/build/logs"
LOG_FILE="${LOG_DIR}/uboot-update.log"
...
echo "========================================="
echo "Updating U-Boot Source"
echo "========================================="
echo "Source Dir: ${UBOOT_SOURCE_DIR}"
echo "Log:        ${LOG_FILE}"
echo ""
...
if [ ! -d "${UBOOT_SOURCE_DIR}" ]; then
    echo "ERROR: U-Boot source directory not found at ${UBOOT_SOURCE_DIR}"
...
if [ -z "${TARGET_TAG}" ]; then
    # Get latest stable tag (v20XX.XX format for U-Boot)
    TARGET_TAG=$(git tag -l 'v20[0-9][0-9].[0-9][0-9]' | sort -V | tail -1)
    if [ -z "${TARGET_TAG}" ]; then
...
if [ "${CURRENT_COMMIT}" = "${TARGET_COMMIT}" ]; then
    echo ""
    echo "✓ U-Boot is already at ${TARGET_TAG}"
    exit 0
fi
...
echo ""
echo "Note: Run ./scripts/build-uboot.sh to rebuild U-Boot"
