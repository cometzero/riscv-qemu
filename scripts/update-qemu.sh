#!/bin/bash
# Update QEMU submodule to latest stable tag
# Usage: ./scripts/update-qemu.sh [--tag <tag>]
...
# Configuration
QEMU_SOURCE_DIR="${PROJECT_ROOT}/sources/qemu"
LOG_DIR="${PROJECT_ROOT}/build/logs"
LOG_FILE="${LOG_DIR}/qemu-update.log"
...
echo "========================================="
echo "Updating QEMU Source"
echo "========================================="
echo "Source Dir: ${QEMU_SOURCE_DIR}"
echo "Log:        ${LOG_FILE}"
echo ""
...
if [ ! -d "${QEMU_SOURCE_DIR}" ]; then
    echo "ERROR: QEMU source directory not found at ${QEMU_SOURCE_DIR}"
...
if [ -z "${TARGET_TAG}" ]; then
    # Get latest stable tag (v*.*.* format, excluding rc versions)
    TARGET_TAG=$(git tag -l 'v[0-9]*.[0-9]*.[0-9]*' | grep -v 'rc' | sort -V | tail -1)
    if [ -z "${TARGET_TAG}" ]; then
...
if [ "${CURRENT_COMMIT}" = "${TARGET_COMMIT}" ]; then
    echo ""
    echo "✓ QEMU is already at ${TARGET_TAG}"
    exit 0
fi
...
echo ""
echo "Note: Run ./scripts/build-qemu.sh to rebuild QEMU"
